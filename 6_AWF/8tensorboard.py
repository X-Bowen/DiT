...
def train_model_worker(params):
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"  Transformer weights: w0={transformer.w0.item():.6f}, w1={transformer.w1.item():.6f}, w2={transformer.w2.item():.6f}")
...
def train_model_worker_wrapper(params, return_dict, idx):
    result = train_model_worker(params)
...
def run_incremental_experiment_parallel():
    ...
    for increment_name in increment_names:
        ...
        for idx, params in enumerate(training_params):
            p = mp.Process(target=train_model_worker_wrapper, args=(params, return_dict, idx))
            p.start()
            processes.append(p)
