...
# BASE_DATA_DIR = "/scratch/bowenxi/dit/neural_tangent_kernel/feature_swin_b/ca/"
BASE_DATA_DIR = "/scratch/bowenxi/dit/neural_tangent_kernel/feature_swin_b/ca/first_3/"
RESULTS_DIR = "incremental_results_2_layer_0530"
NUM_CLASSES = 1200
BATCH_SIZE = 2048
EPOCHS = 1500
#LEARNING_RATES = [1e-3]
LEARNING_RATES = [1e-4, 5e-4, 5e-5]  
#LEARNING_RATES = [5e-5, 1e-4, 3e-4, 7e-4, 1e-3, 5e-3]
# WEIGHT_DECAY = 5e-4
WEIGHT_DECAY =[1e-5, 5e-5, 5e-4]
# WEIGHT_DECAY = 5e-5
MAX_WORKERS = 12
...
class KernelTransformedMLP(nn.Module):
    def __init__(self, input_dim, num_classes, A_matrix):
        super().__init__()
        self.input_dim = input_dim
        self.num_classes = num_classes

        # Register A and its powers (A^2, A^3)
        self.register_buffer('A', A_matrix)
        self.register_buffer('A2', A_matrix @ A_matrix)
        self.register_buffer('A3', self.A2 @ A_matrix)

        # Learnable scalar weights
        self.w0 = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(self, x):
        # Apply kernel transformation: x @ (I + w0*A + w1*A^2 + w2*A^3)
        identity = torch.eye(self.input_dim, device=x.device)
        kernel = identity + self.w0 * self.A + self.w1 * self.A2 + self.w2 * self.A3
        transformed = x @ kernel
        return self.layers(transformed)
...
def train_model_worker(params):
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        num_workers=24,       # Use ~75% of CPUs for data loading
        pin_memory=True,
        prefetch_factor=4     # Increase prefetching to reduce GPU idle time
    )
    ...
    # Load A matrix and move to GPU
    
   # Load the corresponding A matrix for the current increment
    A_path = os.path.join(BASE_DATA_DIR, f"{increment_name}_A_matrix.npy")
    A_np = np.load(A_path)
    A = torch.tensor(A_np, dtype=torch.float32).to(device)

    # Initialize the model with kernel transformation
    model = KernelTransformedMLP(X_train.shape[1], NUM_CLASSES, A).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % 50 == 0:  # Print every 10 epochs to reduce output
    ...
    for increment_name in increment_names:
        ...
        for i, lr in enumerate(LEARNING_RATES):
            for j, wd in enumerate(WEIGHT_DECAY):
                combo_id = i * len(WEIGHT_DECAY) + j
                gpu_id = combo_id % torch.cuda.device_count() if torch.cuda.is_available() else None
...
if __name__ == "__main__":
    ...
    print(f"Testing {len(WEIGHT_DECAY)} weight decays in parallel: {WEIGHT_DECAY}")
